...
const float puckSpeed = 0; //5000.0f;
...
float randomValue(float left = 2500.0f, float right = 15000.0f) {
    std::random_device rd;
    std::mt19937 gen(rd());
    std::uniform_real_distribution distribution(left, right);
    return 0;//distribution(gen);
}
...
int main() {
    ...
    plt::legend();
    plt::title("Сравнение различных стратегий с оптимальными параметрами $exploration-exploitation$");
    plt::xlabel("Раунд");
